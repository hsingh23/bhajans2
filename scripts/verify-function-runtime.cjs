...
const expectedNames = [
  'getUserByEmail', 'updateUserAccess', 'amritabooks',
  'manuallyAddUser', 'shopifyWebhook', 'paidNotification',
];
const deployed = JSON.parse(execFileSync('gcloud', [
...
for (const name of expectedNames) {
  ...
  assert.equal(fn.buildConfig.runtime, 'nodejs24', `${name} has the wrong runtime`);
  assert.equal(fn.serviceConfig.serviceAccountEmail,
...
console.log('Runtime configuration passed. Also verify an authenticated admin lookup in the live UI.');
